...
import { ListingService } from '@bid-scents/shared-sdk'
...
import { queryKeys } from './query-keys'
  ...
  export function useListingDetail(listingId: string) {
    return useQuery({
      ...
      queryFn: () => ListingService.getListingDetailsV1ListingListingIdDetailsGet(listingId),
  ...
  export function useSimpleSearchListings(searchParams: SearchRequest) {
    return useQuery({
      queryKey: queryKeys.listings.search(searchParams),
      queryFn: () => ListingService.searchListingsV1ListingSearchPost(searchParams),
  ...
  export function useUnfavoriteListing() {
    ...
    return useMutation({
      mutationFn: (listingId: string) => 
        ListingService.unfavoriteListingV1ListingListingIdUnfavoriteDelete(listingId),
      onMutate: async (listingId) => {
        ...
        queryClient.setQueriesData<SearchResponse>(
          ...
          (old) => {
            ...
            return {
              ...
              listings: old.listings.map(listing =>
                listing.id === listingId
                  ? { ...listing, favorites_count: Math.max(0, listing.favorites_count - 1) }
                  : listing
              )
            }
          }
  ...
  export function useAddComment() {
    ...
    return useMutation({
      mutationFn: ({ listingId, content }: { listingId: string; content: string }) =>
        ListingService.addCommentV1ListingListingIdCommentPost(listingId, { content }),
      onSuccess: (response: CommentResponse, { listingId }) => {
        // Invalidate listing details to refetch comments
        queryClient.invalidateQueries({ queryKey: queryKeys.listings.detail(listingId) })
      },
    })
  }
  ...
  export function useUpdateComment() {
    ...
    return useMutation({
      mutationFn: ({ commentId, content }: { commentId: string; content: string }) =>
        ListingService.updateCommentV1ListingCommentCommentIdPatch(commentId, { content }),
