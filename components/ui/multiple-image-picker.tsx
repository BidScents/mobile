...
export function MultipleImagePicker({
  ...
  const gap = 12
  const itemsPerRow = 3
  const itemSize = (screenWidth - padding - (gap * (itemsPerRow - 1))) / itemsPerRow
    ...
    try {
      ...
      if (fileInfo.exists && fileInfo.size) {
        ...
        if (sizeInMB > 10) {
          Alert.alert(
            'Image Too Large', 
            `This image is ${sizeInMB.toFixed(1)}MB. Please choose an image smaller than 10MB.`
          )
          return false
        }
      }
  ...
  const moveImage = (fromIndex: number, direction: 'left' | 'right') => {
    ...
    [newUris[fromIndex], newUris[toIndex]] = [newUris[toIndex], newUris[fromIndex]]
    onImagesChange(newUris)
  }
  ...
  return (
    <YStack gap="$3">
      ...
      <YStack gap="$3">
        ...
        {imageUris.length > 0 && (
          <YStack gap="$3">
            {Array.from({ length: Math.ceil(imageUris.length / itemsPerRow) }).map((_, rowIndex) => (
              <XStack key={rowIndex} gap="$3" justifyContent="flex-start">
                {imageUris
                  .slice(rowIndex * itemsPerRow, (rowIndex + 1) * itemsPerRow)
                  .map((uri, indexInRow) => {
                    const globalIndex = rowIndex * itemsPerRow + indexInRow
                    return (
                      <YStack key={globalIndex} gap="$2">
                        <YStack
                          position="relative"
                          width={itemSize}
                          height={itemSize}
                        ...
                        <XStack justifyContent="center" gap="$2">
                          <TouchableOpacity
                            onPress={() => moveImage(globalIndex, 'left')}
                          ...
                          <TouchableOpacity
                            onPress={() => moveImage(globalIndex, 'right')}
                            disabled={globalIndex === imageUris.length - 1}
                            style={{ opacity: globalIndex === imageUris.length - 1 ? 0.3 : 1 }}
                          >
                            <YStack
                              backgroundColor="$background"
                              borderWidth={1}
